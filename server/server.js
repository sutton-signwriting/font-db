...
const server = http.createServer((request, response) => {
  ...
  switch (label) {
    ...
    case "fsw-symbol-normalize":
      fsw.symbolNormalize(parts[4], (err, res) => {
        if (err) {
          console.log(err)
        } else {
    ...
    case "fsw-sign-normalize":
      fsw.signNormalize(parts[4], (err, res) => {
        if (err) {
          console.log(err)
        } else {
          response.writeHead(200, { "Content-Type": "text/plain; charset=utf-8" })
    ...
    case "swu-symbol-normalize":
      swu.symbolNormalize(parts[4], (err, res) => {
        if (err) {
          console.log(err)
        } else {
          response.writeHead(200, { "Content-Type": "text/plain; charset=utf-8" })
    ...
    case "swu-sign-normalize":
      swu.signNormalize(parts[4], (err, res) => {
        if (err) {
          console.log(err)
        } else {
          response.writeHead(200, { "Content-Type": "text/plain; charset=utf-8" })
